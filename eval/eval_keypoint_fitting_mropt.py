...
from score_hmr.utils.loss_utils import limb_sideway_error, limb_torsion_error
from score_hmr.utils.loss_utils import compute_edge_loss, compute_normal_consistency, compute_laplacian_smoothing
...
joint_regressor_extra = torch.tensor(
    pickle.load(open(model_cfg.SMPL.JOINT_REGRESSOR_EXTRA, "rb"), encoding="latin1"),
    dtype=torch.float32,
).to(device)
...
for ii, batch in enumerate(tqdm(dataloader)):
    ...
    # Run iterative refinement with Mesh Refinement Optimization (MROpt)
    pred_pose_rotmat = batch["pred_pose"]  # bs, 24, 3, 3
    pred_pose_6d = pred_pose_rotmat[:, :, :, :2].permute(0, 1, 3, 2)
    all_pose_6d = pred_pose_6d.reshape(batch_size, pred_pose_6d.shape[1], -1)
    all_camera_translation = batch["init_cam_t"]
    all_betas = batch["pred_betas"]

    optimed_pose_6d = all_pose_6d.to(device).clone().detach().requires_grad_(True)  # [bs, 24, 6]
    # optimed_betas = all_betas.to(device).clone().detach().requires_grad_(True)
    optimed_camera_translation = all_camera_translation.to(device).clone().detach().requires_grad_(True)  # [bs, 1, 3]
    # optimed_transl = torch.zeros([batch_size, 3], dtype=torch.float, requires_grad=True, device=device)
    optimed_vert_off_compact = torch.zeros([batch_size, 6890, 3], dtype=torch.float, requires_grad=True, device=device)

    # define the optimizers
    camera_translation_optimizer = torch.optim.Adam([
        {"params": optimed_camera_translation, "lr": 1e-2},
        {"params": optimed_vert_off_compact, "lr": 1e-5}
    ])

    optimizer = torch.optim.SGD([
        {"params": optimed_pose_6d, "lr": 1e-8},
        # {"params": optimed_betas, "lr": 1e-8},
        # {"params": optimed_camera_translation, "lr": 1e-6},
        # {"params": optimed_transl, "lr": 1e-8},
        # {"params": optimed_vert_off_compact, "lr": 1e-8},
    ], momentum=0.9)
    ...
    for i in loop:
        ...
        pred_betas = (
            batch["pred_betas"].unsqueeze(1).repeat(1, NUM_SAMPLES, 1).reshape(batch_size * NUM_SAMPLES, -1)
        )
        pred_pose_rotmat = rot6d_to_rotmat(optimed_pose_6d)
        pred_pose_rotmat = pred_pose_rotmat.view(batch_size, 24, 3, 3)
        pred_smpl_params = {
            "betas": pred_betas,
            "global_orient": pred_pose_rotmat[:, [0]],
            "body_pose": pred_pose_rotmat[:, 1:],
            # "transl": optimed_transl,
            "vert_off_compact": optimed_vert_off_compact,
        }
        smpl_output = diffusion_model.smpl(**{k: v.float() for k, v in pred_smpl_params.items()}, pose2rot=False)

        pred_keypoints_3d = smpl_output.joints
        pred_vertices_3d = smpl_output.vertices
        faces_prep = torch.from_numpy(diffusion_model.smpl.faces).unsqueeze(0).expand((batch_size, -1, -1))

        # Compute the keypoint reprojection error.
        loss_kp = keypoint_fitting_loss(
            model_joints=pred_keypoints_3d,
            camera_translation=optimed_camera_translation,
            joints_2d=batch["joints_2d"],  # from gt
            joints_conf=batch["joints_conf"],  # from gt
            camera_center=batch["camera_center"],  # from gt
            focal_length=batch["focal_length"],  # from gt
            img_size=batch["img_size"],  # from gt
        )

        # loss_limb_sideway = limb_sideway_error(pred_pose_rotmat)
        # loss_limb_torsion = limb_torsion_error(pred_pose_rotmat)
        # loss_edge = compute_edge_loss(pred_vertices_3d, faces_prep)
        # loss_normal = compute_normal_consistency(pred_vertices_3d, faces_prep)
        # loss_laplacian = compute_laplacian_smoothing(pred_vertices_3d, faces_prep)

        # loss_kp: 0.5, loss_limb_sideway: 1, loss_limb_torsion: 10
        loss_total = loss_kp * 0.8

        # Update camera translation.
        camera_translation_optimizer.zero_grad()
        current_optimizer.zero_grad()
        loss_total.sum().backward(retain_graph=True)
        camera_translation_optimizer.step()
        current_optimizer.step()
        loop.set_description("Body Fitting = {:.3f}".format(loss_total.mean().item()))

    dm_out = {'optimed_pose_6d': optimed_pose_6d.detach(),
              # 'optimed_betas': optimed_betas.detach(),
              'optimed_camera_translation': optimed_camera_translation.detach(),
              # "optimed_transl": optimed_transl,
              'optimed_vert_off_compact': optimed_vert_off_compact.detach()
    }

    # Get 3D keypoints after model fitting with ScoreHMR.
    opt_output = {}
    pred_smpl_params = prepare_smpl_params(
        dm_out['optimed_pose_6d'],
    ...
    # pred_smpl_params["transl"] = dm_out["optimed_transl"]
    pred_smpl_params["vert_off_compact"] = dm_out["optimed_vert_off_compact"]
    smpl_out = diffusion_model.smpl(**pred_smpl_params, pose2rot=False)
    opt_output["pred_keypoints_3d"] = pred_keypoints_3d.unsqueeze(1).detach()
