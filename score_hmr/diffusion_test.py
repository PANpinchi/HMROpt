...
class GaussianDiffusion(nn.Module):
    ...
    def model_predictions(
        ...
        if self.objective == "pred_noise":
            ...
            if self.use_guidance and not inverse and self.multiview_guidance:
                timestep = t_[0]
    ...
    @torch.enable_grad()
    def mropt(self, x, batch, batch_size):
        x = x.clone().detach().requires_grad_(True)  # [bs, 24 * 6]

        if self.keypoint_guidance:
            # define the optimizers
            camera_translation_optimizer = torch.optim.Adam([
                {"params": self.camera_translation, "lr": 1e-2},
            ])
            ...
            n_iter = 501  # how many iterations are desired? (+1)
            ...
            for _ in loop:
                if self.keypoint_guidance:
                    # The SMPL betas can only contribute to KP fitting loss.
                    pred_betas = (
                        batch["pred_betas"].unsqueeze(1).repeat(1, 1, 1).reshape(batch_size, -1)
                    )
                    pred_pose_rotmat = rot6d_to_rotmat(x)
                    ...
                    loss_total = (loss_kp * 0.5).sum()

                if self.temporal_guidance:
                    loss_temp = self.cfg.GUIDANCE.W_SMOOTH * smoothness_loss(x)
                    if self.keypoint_guidance:
                        loss_total += loss_total + loss_temp.sum() * 0.5
                    else:
                        loss_total = loss_temp.sum() * 0.5

                    loss_total = loss_total * 0.1
        ...
        if self.multiview_guidance:
            ...
            # start optimizing for this image
            from tqdm import tqdm
            n_iter = 101  # how many iterations are desired? (+1)
            # loop = tqdm(range(n_iter))
            loop = range(n_iter)
            for _ in loop:
                pred_pose_6d = x.reshape(x.size(0), 24, -1)
                loss_mv = multiview_loss(pred_pose_6d[:, 1:])

                # loss_kp: 0.5, loss_limb_sideway: 1, loss_limb_torsion: 10
                loss_total = (loss_mv * 0.5).sum()
    ...
    def ddim_with_guidance(
        ...
        self.optimed_vert_off_compact = torch.zeros([batch_size, 6890, 3], dtype=torch.float,
                                                    requires_grad=True, device=self.device)

        step = 3
        opt_list_outer = [i * (self.optim_iters // step) for i in range(1, step + 1)]
        opt_list_inner = [i * (self.sample_start // step) for i in range(1, step + 1)]
        print('opt_list_outer: ', opt_list_outer)
        print('opt_list_inner: ', opt_list_inner)

        for idx in range(self.optim_iters):
            ...
            for time, time_next in time_pairs:
                ...
                if idx in opt_list_outer and time in opt_list_inner:
                    x_start = self.mropt(x_start, batch, batch_size)
                    x_t = x_start.clone() * alpha_next.sqrt() + c * pred_noise + sigma * noise
