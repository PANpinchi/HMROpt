...
class GaussianDiffusion(nn.Module):
    ...
    def mropt(self, x, batch, batch_size):
        ...
        if self.keypoint_guidance:
            ...
            n_iter = 1501  # how many iterations are desired? (+1)
    ...
    def ddim_with_guidance(
        ...
        if self.multiview_guidance:
            ...
            step = 3
            opt_list = [i * (self.optim_iters // step) for i in range(1, step + 1)]
        print('opt_list: ', opt_list)

        for idx in range(self.optim_iters):

            if idx in opt_list and self.multiview_guidance:
            ...
            if idx in opt_list and not self.multiview_guidance:
                x_start = self.mropt(x_start, batch, batch_size)
                x_t = x_start.clone()
